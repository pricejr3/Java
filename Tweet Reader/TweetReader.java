...
public class TweetReader {
	 ...
	 * 
	 * @param url
	 *            URL of server to retrieve tweets from
	 * @return a list of tweets retrieved from the server.
	 * @throws IOException
	 *             if the url is invalid, the server is unreachable, or some
	 ...
	 * values.
	 */
	private static Map<String, Object> constructTweetMap(JsonValue tree,
			String key) {
	...
	private static Tweet createTweetFromMap(Map<String, Object> tweetMap) {
		...
		ZonedDateTime timestamp = ZonedDateTime.parse(tweetMap
				.get("created_at").toString(), DateTimeFormatter
